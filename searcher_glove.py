...
class Searcher:
    ...
    def search(self, query, k=None):
        ...
        if len(query_as_list) == 0:
            return 0, []
        tuples_tweet_sim = self.relevant_and_cosSim(query_as_list, k)  # CosSim func
        dic_tweet_glove, max_glove_dis = self._model.relative_score_query_docs(query_as_list,
                                                                               tuples_tweet_sim)  # Glove Func
        if tuples_tweet_sim is None:
            return 0, []
        ranked_docs = self._ranker.rank_relevant_doc(tuples_tweet_sim,
                                                     dic_tweet_glove,
                                                     max_glove_dis,
                                                     0.5,
                                                     0.8)
